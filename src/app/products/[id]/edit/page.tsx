...
function EditProduct({ params }: { params: { id: string } }) {
  ...
  const {
    ...
    onSubmit: async (values, actions) => {
      ...
      // added by me
      const _brandsval = values.brands.map((brand) => brand.value).join(",");
      const _occasionval = values.occasion.map((occasion) => occasion.value).join(",");

      const productData: UpdateProducts = {
